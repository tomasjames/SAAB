...
def param_select(params):
    if params['initial_dens']:
        params['initial_dens'] = random.uniform(3, 7)
    if params['b_field']:
...
if __name__ == '__main__':
    ...
    config_file = config.config_file(db_init_filename='database_archi.ini', section='static_fit_results')
    bestfit_config_file = config.config_file(db_init_filename='database_archi.ini', section='static_bestfit_conditions')
    ...
    for obs in filtered_data:
        if (len(obs["species"]) >= 2 and "SIO" in obs["species"]) or \
            ...
            column_names = ["dens", "b_field", "crir", "isrf"]

            print("Running the emcee sampler")
            sampler = mc.EnsembleSampler(nWalkers, nDim, inference.ln_likelihood_static,
            ...
            for counter in range(nBreak):
                ...
                for i in range(0, nWalkers):
                    for j in range(0, nBreak):
                        db.insert_static_chain_data(db_pool=db_pool, table=obs["source"], chain=chain[i][j])
