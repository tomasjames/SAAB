...
def plotting(data, species):
    ...
    vertbar.set_label(
        label=r"$\Delta v_{{{0}}}$ (km/s)".format(species), size=18)
    ...
    # plt.tight_layout()
    fig.savefig('data/images/linewidth_{0}.pdf'.format(species))
...
pixel_height = sgrA_dist * \
    np.deg2rad((max_y_pos - origin_y_pos)/len(image_data[0]))
...
for indx, entry in enumerate(data[1:]):  # data[0] is the header row
    ...
    # Converts arcsec to rad
    source_size_x = sgrA_dist*(source_radial_extent_x*4.84814e-6)

    source_radial_extent_y = float(entry[3])*beam_size_y
    # Converts arcsec to rad
    source_size_y = sgrA_dist*(source_radial_extent_y*4.84814e-6)
